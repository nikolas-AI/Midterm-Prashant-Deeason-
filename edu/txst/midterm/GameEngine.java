...
/**
 * GameEngine manages the game logic for the maze puzzle.
 * It handles player movement, collision detection, and win/loss conditions.
 */
public class GameEngine {
	...
	/**
	 * Initializes the GameEngine with a board.
	 * Automatically finds the player and exit positions.
	 *
	 * @param board The board containing the maze layout
	 */
	...
	/**
	 * Determines if the player has reached the exit.
	 *
	 * @return true if the player is at the exit position, false otherwise
	 */
	...
	/**
	 * Determines if the game is over (player has used all available steps).
	 *
	 * @return true if remaining steps is zero, false otherwise
	 */
	...
	/**
	 * Locates the player's starting position on the board.
	 * Called during initialization to set playerRow and playerCol.
	 */
	 ...
	 * Called during initialization to set exitRow and exitCol.
	 * Checks for both visible and hidden exit cells.
	 */
	...
	/**
	 * Removes the hidden value from all cells on the board.
	 * Called when the player reaches the MAP cell to reveal the entire maze.
	 */
	...
	public void movePlayer(int dRow, int dCol) {
		...
		// 2. Check for Map
		if (targetCell == HIDDEN_MAP || targetCell == MAP) {
			removeHidden();
		}
